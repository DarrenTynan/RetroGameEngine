...
/**
 * @brief Render all the entities onto the screen. The list of entities are held in system (GetSystemEntities)
 */
    ...
    public:
        ...
        void Update(SDL_Renderer* renderer, std::unique_ptr<AssetStore>& assetStore, SDL_Rect& camera)
        {
            ...
            for (auto entity: renderableEntities)
            {
                ...
                SDL_Rect dstRect = {
                    static_cast<int>(transform.position.x - (float)camera.x),
                    static_cast<int>(transform.position.y - (float)camera.y),
//                    static_cast<int>(transform.position.x - (sprite.isFixed ? 0 : camera.x)),
//                    static_cast<int>(transform.position.y - (sprite.isFixed ? 0 : camera.y)),
                    static_cast<int>((float)sprite.width * transform.scale.x),
                    static_cast<int>((float)sprite.height * transform.scale.y)
                };
