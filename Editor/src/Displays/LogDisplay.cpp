...
void EDITOR::LogDisplay::Render(std::shared_ptr<Registry>& registry)
{
    ...
    auto AutoScroll = logger->isAutoScroll();
    ...
    if (ImGui::BeginChild("scrolling", ImVec2(0, 0), ImGuiChildFlags_None, ImGuiWindowFlags_HorizontalScrollbar))
    {
        ...
        if (textFilter.IsActive())
        {
             ...
             * search/filter... especially if the filtering function is not trivial (e.g. regexp).
             */
            for (int line_no = 0; line_no < lineOffsets.Size; line_no++)
            {
                const char* line_start = buf + lineOffsets[line_no];
                const char* line_end = (line_no + 1 < lineOffsets.Size) ? (buf + lineOffsets[line_no + 1] - 1) : buf_end;
                if (textFilter.PassFilter(line_start, line_end))
