...
using namespace EDITOR_FILEDIALOG;
using namespace EDITOR_FILEHANDLER;
...
namespace EDITOR
{
    ...
    void Editor::Setup()
    {
        ...
        editorConfig = fh->LoadConfigFile();
        ...
        TTF_Font *font = TTF_OpenFont("/Users/darren/Development/C++_Projects/RetroGameEditor/fonts/Pixellettersfull.ttf", 32);
...
//        Editor::OpenProjectWindow();
     ...
     */
    void Editor::OpenProjectWindow()
    {
        // Create window with SDL_Renderer graphics context
        auto windowFlags = (SDL_WindowFlags)( SDL_WINDOW_MOUSE_CAPTURE | SDL_WINDOW_ALLOW_HIGHDPI );
        SDL_DisplayMode displayMode;
        SDL_GetCurrentDisplayMode(0, &displayMode);

        SDL_Window *projectWindow = SDL_CreateWindow(
                "Project Select",
        ...
        SDL_Renderer *projectRenderer = SDL_CreateRenderer(projectWindow, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);

        if (!projectRenderer)
        {
            LOGGER::TerminalLogger::Error("Project Window renderer init failed", 0);
            SDL_DestroyRenderer(projectRenderer);
            SDL_Quit();
            exit(1);
        }
        ...
        SDL_SetRenderDrawColor(projectRenderer, 0,0,0,255);
        SDL_RenderClear(projectRenderer);
        ...
        ////////////////
        // Project name
        SDL_Surface *textSurface = TTF_RenderText_Solid(font, editorConfig["project_name"].GetString(), textColor);
        ...
        SDL_Texture *textTexture = SDL_CreateTextureFromSurface(projectRenderer, textSurface);
        ...
        // Rectangle where the text is drawn.
        SDL_Rect textRect = { 150, 50, textSurface->w, textSurface->h };
        SDL_RenderCopy(projectRenderer, textTexture, nullptr, &textRect);

        ///////////////////
        // File path
        SDL_Surface *textSurface1 = TTF_RenderText_Solid(font, editorConfig["file_path"].GetString(), textColor);
        ...
        // Create texture from surface
        SDL_Texture *textTexture1 = SDL_CreateTextureFromSurface(projectRenderer, textSurface1);
        ...
        // Rectangle where the text is drawn.
        SDL_Rect textRect1 = { 150, 100, textSurface1->w, textSurface1->h };
        SDL_RenderCopy(projectRenderer, textTexture1, nullptr, &textRect1);
        ...
        // Load the logo.png
        SDL_Surface* logoSurface = IMG_Load("/Users/darren/Development/C++_Projects/RetroGameEngine/Engine_Test_Game_Platform/icon.png");
        if (logoSurface == nullptr) {
            std::cout << "Error loading image: " << IMG_GetError();
            exit(1);
        }

        SDL_Texture* logoTexture = SDL_CreateTextureFromSurface(projectRenderer, logoSurface);
        ...
        SDL_RenderCopy(projectRenderer, logoTexture, &srcRect, &dstRect);
        ...
        SDL_SetRenderDrawColor( projectRenderer, 128, 128, 128, 255 );

        // Render rect
        SDL_RenderDrawRect( projectRenderer, &boxRect );

        // Render the images to the screen
        SDL_RenderPresent(projectRenderer);
...
//                        break;
        ...
        SDL_DestroyRenderer(projectRenderer);
        SDL_DestroyWindow(projectWindow);
        SDL_Quit();
    }


    /**
     * @brief Setup the main editor window.
     */
    void Editor::OpenEditorWindow()
    {
        SDL_DisplayMode displayMode;
        SDL_GetCurrentDisplayMode(0, &displayMode);

        // Create window with SDL_Renderer graphics context
        auto windowFlags = (SDL_WindowFlags)( SDL_WINDOW_RESIZABLE | SDL_WINDOW_MOUSE_CAPTURE | SDL_WINDOW_MAXIMIZED | SDL_WINDOW_ALLOW_HIGHDPI );
